# Untitled_Knight_Game_Compatibility_Mod
<p>This is only meant to be an example mod</p>

Untitled_Knight_Game_Compatibility_Mod
Copyright (C) 2026 Daniel Hanrahan Tools and Games
This Expansion or Mod is under the BSD-3-Clause license and everything else in this Expansion/Mod is under this: This work is licensed under CC BY-SA 4.0

<h3>Both of these links have the mod support information needed</h3>

<a href="https://github.com/Daniel-Hanrahan-Tools-and-Games/Untitled_Knight_Game_Compatibility">Compatibility Version Repository Page</a>

<a href="https://daniel-hanrahan-tools-and-games.github.io/Untitled_Knight_Game_Compatibility/">Compatibility Version ReadMe Page</a>





This is for stuff not covered by BSD-3-Clause:
CC BY-SA 4.0 Conditional Exceptions CC BY 4.0:

If **either** of the following conditions is met, the licensing
rules for both content covered by CC BY-SA 4.0 are modified as described below:

Conditions:

1. The developer has made a licensing agreement with another entity
   (excluding corporate/LLC or equivalent entities) that prohibits the
   redistribution of content under copyleft licenses.

2. The developer adds assets that they do not own and uses them
   under fair use, or equivalent/similar legal arrangements, where
   the developer cannot legally contribute those assets under a
   copyleft license.

3. The developer is distributing, porting, or integrating the software with platforms or environments that impose requirements incompatible with CC BY-SA 4.0, including but not limited to:
- proprietary or non-redistributable SDKs
- confidential hardware or platform documentation
- legally required confidentiality obligations preventing full CC BY-SA 4.0 redistribution
- safety-regulated or certified systems where full CC BY-SA 4.0 redistribution cannot be satisfied

Effect on licensing:

- Content covered by CC BY-SA 4.0: Assets originally under CC BY-SA 4.0
  may instead be used under CC BY 4.0.

These exceptions apply **only when at least one of the conditions above is met**.


